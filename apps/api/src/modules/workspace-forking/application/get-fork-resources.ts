...
import type { WorkspaceForkResourceCatalogReader } from '@/modules/workspace-forking/ports/workspace-fork-resource-catalog-reader'

export type GetForkResourcesResult =
  | { ok: true; value: GetForkResourcesResponseV1 }
...
export interface GetForkResourcesUseCase {
  execute(
    context: AuthenticatedRequestContext,
    workspaceId: string
  ): Promise<GetForkResourcesResult>
}

export interface GetForkResourcesDependencies {
...
export function createGetForkResourcesUseCase(
  dependencies: GetForkResourcesDependencies
): GetForkResourcesUseCase {
  return {
    async execute(context, workspaceId) {
      ...
      const resources = await dependencies.catalog.readCopyable(params.data.id)
      return {
        ok: true,
        value: getForkResourcesResponseV1Schema.parse(resources),
      }
    },
  }
}
