...
import type { ForkEntitlementReader } from '@/modules/workspace-forking'

const logger = createLogger('ForkEntitlement')

export function createDrizzleForkEntitlementReader(
    ...
    async isEntitled(organizationId) {
      ...
      try {
        ...
        if (owner) {
          ...
          logger.error('Organization has no owner while checking fork entitlement', {
            organizationId,
          })
        }
        ...
        logger.error('Failed to check workspace fork entitlement', {
          error,
          organizationId,
        })
        return false
