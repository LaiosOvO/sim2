...
| W2 tenant-read 22 路：`invitations`、`organizations`、`permission-groups`、`stars`、`users`、`workspace-events`、`workspaces` | `packages/api-contracts/src/w2-tenant-read.ts` + `apps/api/src/modules/tenant-read` + 原路径 generated facade | S | 兼容平面已完成；upstream 修改这些旧 route 时，先更新版本化契约和旧/新差分 fixture，不得把旧 route 实现合回 Next；当前 `API-0294 stars` 已 native，其余 21 条仍是固定 legacy origin，按 coverage 中的 inventory ID 逐条替换 | W2 22/22 C/A/D/I、backend status、generated check、facade bundle、tenant isolation |
