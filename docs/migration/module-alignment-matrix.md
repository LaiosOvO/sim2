...
| W2 tenant-read 22 路：`invitations`、`organizations`、`permission-groups`、`stars`、`users`、`workspace-events`、`workspaces` | `packages/api-contracts/src/w2-tenant-read.ts` + `apps/api/src/modules/tenant-read` + 领域 Module + 原路径 generated facade | S | 兼容平面已完成；upstream 修改这些旧 route 时，先更新版本化契约和旧/新差分 fixture，不得把旧 route 实现合回 Next；当前 `API-0137 invitations`、`API-0209 data drain runs`、`API-0235 organization roster`、`API-0241 organization workspaces`、`API-0243 user permission group`、`API-0294 stars`、`API-1009 background work`、`API-1031 fork availability`、`API-1034 fork lineage`、`API-1037 fork resources`、`API-1041 workspace host context`、`API-1057 workspace members`、`API-1058 workspace execution metrics`、`API-1060 Polaris personal profile`、`API-1124 workspace invitations` 已 native，其余 7 条仍是固定 legacy origin，按 coverage 中的 inventory ID 逐条替换；API-1009/API-1037 在独立 agent 审查通过前不进入严格验收计数 | W2 22/22 C/A/D/I、backend status、generated check、facade bundle、tenant isolation、independent review |
| `apps/sim/app/api/invitations/route.ts` + `apps/sim/lib/invitations/core.ts#listPendingInvitationsForEmail` | `packages/api-contracts/src/invitations.ts` + `apps/api/src/modules/invitations/**` + `apps/api/src/infrastructure/postgres/repositories/drizzle-invitation-read-repository.ts` | S | `API-0137` 已原生化；upstream 若修改 invitee response、email/status/expiry filter 或 grant hydration，先更新 V1 contract 与旧/新 fixture，再适配两批查询；禁止恢复 token 字段或把旧 invitation core import 到 API | contract token stripping、session auth、wire differential、disposable PostgreSQL integration |
| `apps/sim/app/api/workspaces/invitations/route.ts` + `apps/sim/lib/invitations/core.ts#listInvitationsForWorkspaces` + `lib/workspaces/utils.ts#listAccessibleWorkspaceRowsForUser` | `packages/api-contracts/src/invitations.ts` + Invitations Module management read port + shared Drizzle invitation adapter | S | `API-1124` 已原生化；upstream 修改 token/状态字段、显式 permission、组织管理员派生规则或 archive filter 时同步更新 management contract 与 real-DB fixture；不得与 API-0137 token-free DTO 合并 | session auth、13-field wire differential、explicit/derived/no-access/archived PostgreSQL cases |
| `apps/sim/app/api/workspaces/[id]/members/route.ts` + `lib/workspaces/permissions/utils.ts#getWorkspaceMemberProfiles` | `packages/api-contracts/src/workspaces.ts` + `apps/api/src/modules/workspaces/**` + `apps/api/src/infrastructure/postgres/repositories/drizzle-workspace-member-read-repository.ts` | S | `API-1057` 已原生化；upstream 若修改 effective workspace access 或轻量 member 字段，分别更新 authorization fixture 与 response contract；不得把组织管理员派生权限误当显式成员，也不得恢复旧 permission utility import | session auth、cross-workspace/archived 404、org-admin inheritance、real PostgreSQL member differential |
| `apps/sim/app/api/organizations/[id]/workspaces/route.ts` + permission-group `authorizeOrgAccessControl/listOrganizationWorkspaces` | `packages/api-contracts/src/organizations.ts` + `apps/api/src/modules/organizations/**` + organization PostgreSQL adapters | S | `API-0241` 已原生化；upstream 修改 admin role、self-host flag、billing-block/subscription entitlement、workspace sort/archive 语义时同步更新窄 ports 与 real-DB fixture；禁止把 Billing/NextResponse barrel 搬入 application | session auth、admin/enterprise 403、generic 500、cloud/self-host/billing-disabled DB cases |
| `apps/sim/app/api/organizations/[id]/roster/route.ts` + `lib/invitations/core.ts#expireStalePendingInvitationsForOrganization` | `packages/api-contracts/src/organizations.ts` + Organizations roster use case/read snapshot/housekeeping ports + Drizzle adapters | S | `API-0235` 已原生化；upstream 修改 member/admin projection、external grouping、workspace permission 或 invitation expiry 时同步更新 contract、snapshot adapter 与 real-DB fixture；GET 写副作用必须继续走显式 housekeeping port，禁止恢复 Invitations Core/Drizzle/NextResponse import；跨 organization 或 archived grant 不兼容保留 | member redaction、admin projection、best-effort cleanup、earliest external permission、tenant-scoped PostgreSQL grants |
| `apps/sim/app/api/permission-groups/user/route.ts` + `ee/access-control/utils/permission-check.ts#resolveWorkspaceGroup` | `packages/api-contracts/src/permission-groups.ts` + `apps/api/src/modules/permission-groups/**` + `apps/api/src/infrastructure/postgres/repositories/drizzle-user-permission-group-read-repository.ts` | S | `API-0243` 已原生化；upstream 修改 config 默认值、auth type、workspace access、enterprise entitlement 或 group precedence 时同步更新纯 contract normalizer、use case、adapter 与 real-DB fixture；禁止把旧 EE permission-check、Executor/Block/Provider parser、Billing barrel 或 runtime registry 合回 metadata 读取链 | auth/query/404/403 顺序、personal/non-entitled 短路、owning-org scope、explicit > empty > default、oldest tie-break、config stripping |
| `apps/sim/app/api/workspaces/[id]/host-context/route.ts` + `lib/workspaces/host-context.ts` + Billing workspace payer/access helpers | `packages/api-contracts/src/workspaces.ts` + Workspaces host-context use case/snapshot port + `drizzle-workspace-host-context-read-repository.ts` | S | `API-1041` 已原生化；upstream 修改 workspace mode/host fields、effective permission、organization membership、subscription status/priority、billing interval 或 payer block 时同步更新 V1 contract、application projection、snapshot adapter 与 real-DB fixture；禁止恢复 React cache、Next、legacy Billing Core 或 active-organization 推断；API-1011/API-1122 必须复用该 seam | session/403 concealment、external/admin projection、org newest、personal enterprise>team>pro、owner/personal block、archive |
| `apps/sim/app/api/workspaces/[id]/metrics/executions/route.ts` + workflow/log/paused-execution SQL | `packages/api-contracts/src/workspaces.ts` + Workspaces execution-metrics use case/read port + `drizzle-workspace-execution-metrics-read-repository.ts` | S | `API-1058` 已原生化；upstream 修改 query defaults、folder/workflow/trigger/level semantics、pending detection、all-time bounds、bucket edge、success/avg/percentile projection 时同步更新 V1 schema、纯 application 规则、adapter 和 real-DB fixture；不得把 DB/Drizzle、execution payload、Executor/Registry/Sandbox 搬回 Module 或 facade；aggregate timestamp 必须保持 UTC | query/error differential、tenant access、filters、no-data/all-time、bucket/percentiles、paused join、PostgreSQL UTC |
| `apps/sim/app/api/workspaces/[id]/fork/availability/route.ts` + `ee/workspace-forking/lib/lineage/authz.ts` + `lib/core/config/feature-flags.ts|appconfig.ts` | `packages/api-contracts/src/workspace-forking.ts` + `apps/api/src/modules/workspace-forking/**` + Fork context/entitlement/admin PostgreSQL adapters + `extensions/infra/appconfig` | S | `API-1031` 已原生化；upstream 修改 self-host flag precedence、Enterprise subscription、owner billing-block、host detection、AppConfig profile/cache/rule、platform-admin 定义或 availability 错误折叠时，分别同步到 runtime config、窄 port/adapter、AppConfig Infra 与差分 fixture。donor 未检查 `hasAccess`，当前兼容这一布尔可见性；若收紧为成员可读必须独立决策。禁止把全局 feature registry、Billing Core、AWS SDK、Executor/Registry/Sandbox 搬入 Module 或 facade | session/404/wire differential、deployment/plan/rollout fail-closed、global/user/org/admin rule、cache/last-good、real PostgreSQL、browser closure |
| `apps/sim/app/api/workspaces/[id]/fork/lineage/route.ts` + `apps/sim/ee/workspace-forking/lib/lineage/lineage.ts|authz.ts` + `apps/sim/lib/workspaces/permissions/utils.ts` + `apps/sim/ee/workspace-forking/lib/promote/promote-run-store.ts` | `packages/api-contracts/src/workspace-forking.ts` + Workspace Forking shared gate/current-access/lineage ports + `drizzle-workspace-fork-current-access-reader.ts` + `drizzle-workspace-fork-lineage-reader.ts` + `packages/platform-authz/src/workspace.ts` batch access resolver | S | `API-1034` 已原生化；upstream 修改 gate/admin 错误优先级、active parent/child 过滤、children 排序、有效权限继承、promote-run latest 选择或缺失 source 名称回退时，同步更新共享 policy、viewer-specific read model、V1 contract 与差分/真实数据库 fixture。禁止恢复 route 对 parent/child 逐项权限读取的 N+1，也禁止把 DB schema、Billing Core、feature registry 或 lineage helper 暴露给 Web facade | session/validation/gate/admin 顺序、parent/children/undo wire、explicit/inherited/hidden access、archived/fallback PostgreSQL cases、固定批次权限投影、native routing、browser closure |
| `apps/sim/app/api/workspaces/[id]/fork/resources/route.ts` + `apps/sim/ee/workspace-forking/lib/copy/copy-resources.ts` + resource schemas | `packages/api-contracts/src/workspace-forking.ts` + Workspace Forking resource-catalog port + `drizzle-workspace-fork-resource-catalog-reader.ts` | S | `API-1037` 已完成 native 实现并等待独立审查；upstream 修改 7 类资源筛选、file folder join、1,000 上限、deployed workflow active-version/excluded/archive 语义或响应字段时，同步更新 V1 contract、catalog adapter、focused/real-DB fixture；禁止把 MCP secrets/headers、custom-tool code/schema、Skill content、Registry、Executor 或 Sandbox 暴露给 API contract/Web | session/validation/gate/admin 顺序、8 个固定查询、folder deletion/type cases、secret stripping、deployed count、real PostgreSQL、native routing、browser closure、independent review |
| `apps/sim/app/api/workspaces/[id]/background-work/route.ts` + `apps/sim/ee/workspace-forking/lib/promote/promote-run-store.ts` + `apps/sim/ee/workspace-forking/hooks/background-work.ts` | `packages/api-contracts/src/workspace-background-work.ts` + Workspace Forking background-work read port + `drizzle-workspace-background-work-reader.ts` + focused Sim contract wrapper | S | `API-1009` 已完成 native 实现并等待独立审查；upstream 修改 involvement OR 条件、live-child active filter、kind/status allowlist、keyset cursor、metadata allowlist 或 gate/admin 错误顺序时，同步到 V1 contract、use case、PostgreSQL adapter 与差分/真实数据库 fixture。Web 只能依赖 focused wrapper，禁止重新导入 800+ 行 workspace-fork monolith、DB、Registry、Executor 或 Sandbox | session/validation/gate/admin 顺序、direct/metadata/live-child tenant cases、keyset pagination、metadata secret stripping、real PostgreSQL、native routing、focused/full Sim type-check、browser closure、independent review |
| `apps/sim/app/api/organizations/[id]/data-drains/[drainId]/runs/route.ts` + `lib/data-drains/access.ts` + `serializers.ts` | `packages/api-contracts/src/data-drains.ts` + `apps/api/src/modules/data-drains/**` + `drizzle-data-drain-entitlement-reader.ts` + `drizzle-data-drain-run-read-repository.ts` | S | `API-0209` 已原生化；upstream 修改 self-host feature precedence、Enterprise entitlement、owner/admin gate、run fields/status/trigger、limit/default、sort 或 locator serialization 时同步更新 runtime config、两个窄 ports、V1 contract 与 real-DB fixture；destination config/dispatcher registry、legacy Billing helper、Next/Auth/Executor/Sandbox 不得进入读取链 | auth/gate/validation 顺序、1..200/default 25、wire projection、org-scoped drain、newest-first、billing-block、boundary/build |
| `apps/sim/app/api/workspaces/[id]/credit-availability/route.ts` + `usage-gate/route.ts` + `lib/billing/**#checkAttributedUsageLimits` | `apps/api/src/modules/billing/read-model/**`（待建）+ Workspaces host/payer seam + `packages/api-contracts/src/billing.ts`（待建） | S | `API-1011/API-1122` 暂保持 legacy；先版本化 attribution snapshot、payer/member ledger、daily refresh、plan/goodwill/on-demand limit 纯规则，并分离 enforcement 与 display projection；upstream 修改 payer attribution、block、账期、pool/member cap 或 fail-open/fail-closed 时同步到该 foundation 的对应 port/rule/fixture，禁止复制 600+ 行 Billing Core | donor/native differential、payer immutability、period boundary、pool/member caps、daily refresh、failure policy、real PostgreSQL |
...
| `apps/sim/lib/core/config/appconfig.ts` + `appconfig-rules.ts` | `extensions/infra/appconfig` + feature-owned API composition | S |
...
| --- | --- | --- | --- | --- |
| `app/api/workspaces/[id]/personal-profile` + `lib/polaris/identity/personal-profile` + `externalIdentity` schema | `packages/api-contracts/src/workspaces.ts` + `extensions/biz/identity` + `apps/api/src/modules/identity` + PostgreSQL adapter + target migration 0275 | D/S | API-1060 read wire、workspace authorization、个人账号/workspace/外部身份读取；当前已迁移 | 复制 donor migration 编号；Biz 暴露 Feishu alias；返回 raw profile |
